...
def main() -> None:
    ...
    data_dir = Path(
        os.environ.get("Q3_CONTINUE_DATA_DIR", "")
        or os.environ.get("Q3_DATA_DIR", "")
        or (repo_root / "cleaned_data")
    )
    q1_run_dir = Path(
        os.environ.get("Q3_CONTINUE_Q1_RUN_DIR", "")
        or os.environ.get("Q3_Q1_RUN_DIR", "")
        or (repo_root / "outputs" / "q1" / "run_20260425_182112")
    )
    events_path = Path(
        os.environ.get("Q3_CONTINUE_EVENTS_PATH", "")
        or os.environ.get("Q3_EVENTS_PATH", "")
        or (data_dir / "q3_events_reasonable_samples_from_q1_run_20260425_182112.json")
    )

    output_root = Path(
        os.environ.get("Q3_CONTINUE_OUTPUT_DIR", "")
        or os.environ.get("Q3_OUTPUT_DIR", "")
        or (repo_root / "outputs" / "q3_continue")
    )
    output_timestamp = (
        os.environ.get("Q3_CONTINUE_OUTPUT_TIMESTAMP", "")
        or os.environ.get("Q3_OUTPUT_TIMESTAMP", "1")
    )
    if output_timestamp != "0":
