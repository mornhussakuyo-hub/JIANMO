...
@dataclass(slots=True)
class Q3RollingState:
    input_data: Q1InputData
    customers: dict[int, Customer]
    service_units: dict[str, ServiceUnit]
    routes: list[Route]
    synthetic_original_customer: dict[int, int] = field(default_factory=dict)


class Q3DynamicSolver:
    """Q3 连续滚动入口：按事件时间顺序推进系统状态。"""
    ...
    def solve_all(self) -> list[Q3CaseResult]:
        ...
        results: list[Q3CaseResult] = []
        for index, event in enumerate(events, start=1):
            log(
                f"连续滚动进度 {index}/{len(events)}: "
                f"{event['event_id']} @ {float(event['event_time_min']):.0f}",
                indent=1,
            )
            result, state = solve_event_case_from_state(
                state=state,
                event=event,
        ...
        log(f"Q3 连续滚动求解完成: {ok_count}/{len(results)} 个事件时点通过校验", indent=1)
    ...
    def _build_initial_state(self) -> Q3RollingState:
        log("加载连续滚动初始状态: 读取 Q1 输入与基准路线", indent=1)
        input_data = Q1DataLoader(data_dir=self.data_dir, constants=self.constants).load()
        ...
        routes = _load_q1_routes(
            q1_run_dir=self.q1_run_dir,
            base_units={unit.unit_id: unit for unit in base_units},
        )
        return Q3RollingState(
...
def solve_event_case_from_state(
    state: Q3RollingState,
    event: dict[str, Any],
    constants: Q1Constants | None = None,
) -> tuple[Q3CaseResult, Q3RollingState]:
    constants = constants or Q1Constants()
    event_id = str(event["event_id"])
    event_type = str(event["event_type"])
    event_time_min = float(event["event_time_min"])
    log(f"处理连续事件 {event_id}: {event_type}, t={event_time_min:.0f}", indent=2)

    base_routes = _copy_routes(state.routes)
    base_assignment = _unit_assignment(base_routes)

    customers = _copy_customers(state.customers)
    service_units = {
        unit_id: replace(unit, time_window=replace(unit.time_window), source_order_ids=list(unit.source_order_ids))
        for unit_id, unit in state.service_units.items()
    }
    routes = _copy_routes(state.routes)
    synthetic_original_customer = dict(state.synthetic_original_customer)
    notes: list[str] = []
    added_route_count = 0
    removed_route_count = 0

    pre_event_evaluator = _build_evaluator(
    ...
    frozen_unit_ids = _frozen_service_units_from_routes(
        routes=routes,
        evaluator=pre_event_evaluator,
    ...
    if event_type == "cancel_order":
        ...
        if affected_unit is None:
            ...
            notes.append(f"取消事件目标已冻结，保留当前连续状态: {unit_id}")
        else:
            ...
            notes.append(f"取消连续状态中的未来服务单元: {unit_id}")
        ...
        if old_unit is None:
            ...
            notes.append(f"变更事件目标已冻结，保留当前连续状态: {unit_id}")
        else:
        ...
        notes.append(f"未知事件类型，保留当前连续状态: {event_type}")
    ...
    residual_unit_count = len(service_units) - frozen_unit_count
    ...
    result = Q3CaseResult(
        ...
        frozen_unit_count=frozen_unit_count,
...
def _frozen_service_units_from_routes(
    routes: list[Route],
    evaluator: RouteEvaluator,
    event_time_min: float,
) -> set[str]:
    """
    基于当前连续状态中的路线重新判定冻结集合。

    连续滚动版不能再回到 Q1 原始 CSV 切片，
    必须根据上一事件后的当前路线时刻来判断哪些服务单元已经完成，
    哪些正处在行驶/服务中的保守冻结区间。
    """
    ...
    for route in routes:
        ...
        if not evaluation.feasible:
            continue
        for stop, leg in zip(route.stops, [leg for leg in evaluation.leg_records if leg.to_node != 0]):
            if leg.leave_min <= event_time_min:
                frozen.update(stop.service_unit_ids)
                continue
            if float(route.departure_min) <= event_time_min < leg.leave_min:
                frozen.update(stop.service_unit_ids)
                break
    return frozen
