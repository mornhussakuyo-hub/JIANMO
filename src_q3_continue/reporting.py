...
class Q1ReportBuilder:
    ...
    def write_all(self, solution: Solution, output_dir: Path) -> dict[str, Path]:
        """写出完整 Q1 结果文件，并返回文件路径。"""

        output_dir.mkdir(parents=True, exist_ok=True)
        payload = self.build_summary_dict(solution)

        paths = {
            "json": output_dir / "q1_solution.json",
            "kpi_csv": output_dir / "q1_kpi.csv",
            "routes_csv": output_dir / "q1_routes.csv",
            "route_arcs_csv": output_dir / "q1_route_arcs.csv",
            "route_stops_csv": output_dir / "q1_route_stops.csv",
            "customer_service_csv": output_dir / "q1_customer_service.csv",
            "vehicle_usage_csv": output_dir / "q1_vehicle_usage.csv",
            "report_md": output_dir / "q1_report.md",
        }
    ...
    def _build_markdown(self, payload: dict[str, Any]) -> str:
        ...
        lines.append("## 5. 文件说明")
        lines.append("")
        lines.append("- `q1_route_arcs.csv`：正式路线弧段表，含真实出发/到达/服务/离开时刻。")
        lines.append("- `q1_route_stops.csv`：每条路线的客户停靠与配送量。")
        lines.append("- `q1_customer_service.csv`：客户层服务完成情况。")
        lines.append("- `q1_vehicle_usage.csv`：车辆复用与车辆成本汇总。")
        return "\n".join(lines)
    ...
    def _write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
        ...
        with path.open("w", newline="", encoding="utf-8-sig") as file:
            ...
            writer.writeheader()
            for row in rows:
                writer.writerow({key: Q1ReportBuilder._csv_value(row.get(key)) for key in fieldnames})
