...
def _departure_candidates_for_schedule(route: Route, route_evaluator) -> list[int]:
    ...
    for stop in route.stops:
        ...
        has_green_stop = has_green_stop or customer.is_green
        for value in [
    ...
    upper = min(24 * 60 - 1, max(1080 if has_green_stop else 1020, latest_window_end + 120))
    for value in [480, 540, 600, 690, 780, 900, 930, 960, 990, 1020, 1050, 1080]:
        if 480 <= value <= upper:
            candidates.add(value)

    step = 15 if has_green_stop else 30
    for value in range(480, upper + 1, step):
        candidates.add(value)
