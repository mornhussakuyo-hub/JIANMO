...
def _initial_candidate_worker(
    build_kind: str,
    route_evaluator: RouteEvaluator,
    ordered_units: Sequence[ServiceUnit],
    vehicles: Sequence[VehicleInstance],
) -> Solution:
    """初始解候选的进程 worker。"""

    builder = InitialSolutionBuilder(route_evaluator=route_evaluator)
    if build_kind == "insertion":
        return builder._build_from_order(ordered_units, vehicles)
    if build_kind == "split_dp":
        return builder.split_dp_builder.build_solution(ordered_units, vehicles)
    raise ValueError(f"未知初始解构造模式: {build_kind}")
...
class InitialSolutionBuilder:
    ...
    def __init__(self, route_evaluator: RouteEvaluator) -> None:
        ...
        self.parallel_initial_enabled = os.environ.get(
            "Q1_MC_ENABLE_INITIAL",
            os.environ.get("Q1_MC_ENABLE", "1"),
        ) != "0"
        self.parallel_initial_workers = max(
            1,
            int(os.environ.get("Q1_MC_INITIAL_WORKERS", os.environ.get("Q1_MC_WORKERS", "4")) or "4"),
        )
    ...
    def build(self, service_units: Sequence[ServiceUnit], vehicles: Sequence[VehicleInstance]) -> Solution:
        ...
        candidate_jobs: list[tuple[str, str, list[ServiceUnit]]] = []
        for name, ordered_units in candidate_orders:
            if not skip_insertion:
                candidate_jobs.append((f"{name} / insertion", "insertion", ordered_units))
            if name != "hardness_sorted":
                candidate_jobs.append((f"{name} / split_dp", "split_dp", ordered_units))

        candidate_solutions = self._build_candidate_solutions(candidate_jobs, vehicles)
        for label, solution in candidate_solutions:
            self._log_solution_summary(label, solution, indent=3)
            if best_solution is None or self._solution_rank(solution) < self._solution_rank(best_solution):
                best_solution = solution
                log(f"当前最优更新为: {label}", indent=3)

        if best_solution is not None:
            self._log_solution_summary("初始解最终选择", best_solution, indent=2)
        return best_solution or Solution()

    def _build_candidate_solutions(
        self,
        candidate_jobs: Sequence[tuple[str, str, list[ServiceUnit]]],
        vehicles: Sequence[VehicleInstance],
    ) -> list[tuple[str, Solution]]:
        if not candidate_jobs:
            return []

        if not self.parallel_initial_enabled or len(candidate_jobs) <= 1:
            return [
                (label, self._build_candidate_solution_sequential(build_kind, ordered_units, vehicles))
                for label, build_kind, ordered_units in candidate_jobs
            ]

        worker_count = min(self.parallel_initial_workers, len(candidate_jobs))
        log(
            f"初始解候选并行启动: 任务 {len(candidate_jobs)} 个, workers={worker_count}",
            indent=2,
        )

        results: dict[int, tuple[str, Solution]] = {}
        with ProcessPoolExecutor(
            ...
            future_map = {
                executor.submit(
                    _initial_candidate_worker,
                    build_kind,
                    self.route_evaluator,
                    ordered_units,
                    list(vehicles),
                ): (index, label, build_kind, ordered_units)
                for index, (label, build_kind, ordered_units) in enumerate(candidate_jobs)
            }

            for future in as_completed(future_map):
                index, label, build_kind, ordered_units = future_map[future]
                try:
                    solution = future.result()
                    log(f"并行候选完成: {label}", indent=3)
                except Exception as exc:
                    log(f"并行候选失败，回退串行: {label}, 原因={exc}", indent=3)
                    solution = self._build_candidate_solution_sequential(build_kind, ordered_units, vehicles)
                results[index] = (label, solution)

        return [results[index] for index in range(len(candidate_jobs))]

    def _build_candidate_solution_sequential(
        self,
        build_kind: str,
        ordered_units: Sequence[ServiceUnit],
        vehicles: Sequence[VehicleInstance],
        ...
        if build_kind == "insertion":
            return self._build_from_order(ordered_units, vehicles)
        if build_kind == "split_dp":
            return self.split_dp_builder.build_solution(ordered_units, vehicles)
        raise ValueError(f"未知初始解构造模式: {build_kind}")
